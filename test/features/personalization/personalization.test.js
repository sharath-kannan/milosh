...
import { init, matchGlob, applyPers, combineNonTargetSources } from '../../../libs/features/personalization/personalization.js';
...
describe('MEP Utils', () => {
  describe('combineNonTargetSources', async () => {
    it('yields an empty list when everything is undefined', async () => {
      const manifests = await combineNonTargetSources(undefined, undefined, undefined);
    ...
    it('combines promos and personalization', async () => {
      ...
      const manifests = await combineNonTargetSources('/pers/manifest.json', 'pre-black-friday-global,black-friday-global', undefined);
    ...
    it('combines promos and personalization and mep param', async () => {
      document.head.innerHTML = await readFile({ path: '../../utils/mocks/mep/head-promo.html' });
      const manifests = await combineNonTargetSources(
        '/pers/manifest.json',
